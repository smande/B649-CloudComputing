	...
	public void map(LongWritable key, Text value, Context context)
		throws IOException, InterruptedException {
			...
			if (rrd.targetUrlsList.size()<=0){
				...
				double rankValforTargetUrl = 0d;
				int numOfTargetUrls = rrd.targetUrlsList.size();
				System.out.println ("Node:" + rrd.sourceUrl + "has targetURLsize of:" + numOfTargetUrls);
				for (int i=0;i<numOfTargetUrls;i++)	{
					targetUrl = rrd.targetUrlsList.get(i);
					rankValforTargetUrl = rrd.rankValue/numOfTargetUrls;
					System.out.println ("TargetUrl:" + targetUrl + "has a page rank Value of:" + rankValforTargetUrl);
					context.write(new LongWritable(targetUrl), new Text(Double.toString(rankValforTargetUrl)));
					   sb.append("#" + Integer.toString(targetUrl));					
				}
			} //for
